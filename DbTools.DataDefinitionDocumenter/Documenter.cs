    ...
    : DocumenterWriterBase(documenterWriter, context, version, databaseName, fileName)
{
    ...
    public void Document(IDatabaseDefinition databaseDefinition)
    {
        ...
        foreach (var table in tables)
        {
            ...
            if (!Customizer.ShouldSkip(schemaAndTableName))
                _sqlTablesByCategory.Add(new KeyValuePair<string?, SqlTable>(Customizer.Category(schemaAndTableName), table));
            else
                _skippedSqlTablesByCategory.Add(new KeyValuePair<string?, SqlTable>(Customizer.Category(schemaAndTableName), table));
        }
        ...
        foreach (var view in views)
        {
            ...
            if (!Customizer.ShouldSkip(schemaAndTableName))
                _sqlViewsByCategory.Add(new KeyValuePair<string?, SqlView>(Customizer.Category(schemaAndTableName), view));
            else
                _skippedSqlViewsByCategory.Add(new KeyValuePair<string?, SqlView>(Customizer.Category(schemaAndTableName), view));
        }
